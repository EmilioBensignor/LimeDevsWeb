export const projects = [
  {
    slug: "cladd-industria-textil",
    img: "Cladd",
    alt: "Cladd Industria Textil - Software y Hardware para la industria textil",
    title: "Cladd Industria Textil",
    service: "Software and Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "Cladd Industria Textil - Software y soluciones de hardware",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    description: "Custom software and hardware systems to optimize processes in the textile industry. Automation solutions to improve production efficiency.",
    keywords: ["industria textil", "automatización", "software", "hardware", "optimización de procesos", "argentina"],
    publishDate: "2023-03-15T00:00:00Z",
    type: "software-hardware",
    client: "Cladd Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-image.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "mitzvah-for-israel",
    img: "Mitzvah-for-israel",
    alt: "Mitzvah for Israel - Plataforma digital de donaciones",
    title: "Mitzvah for Israel",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    description: "Digital platform to facilitate donations to Israel. Modern and accessible software design that connects people around the world with humanitarian causes.",
    keywords: ["donaciones", "plataforma digital", "israel", "causas humanitarias", "software", "caridad"],
    publishDate: "2023-02-10T00:00:00Z",
    type: "software",
    client: "Mitzvah Foundation",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "unity-box",
    img: "UnityBox",
    alt: "Unitybox",
    title: "Unitybox",
    service: "Software and Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    description: "Digital donation platform that transforms the way of giving through a digital charity box system. Facilitating donations with just a touch.",
    keywords: ["donaciones", "caridad", "plataforma digital", "caja de caridad", "fintech", "impacto social"],
    publishDate: "2022-06-20T00:00:00Z",
    type: "software-hardware",
    client: "Unitybox Foundation",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "cteen-summer",
    img: "Cteen",
    alt: "Cteen",
    title: "Cteen Summer",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    description: "Software for managing summer activities for youth. Platform for registration, tracking and coordination of youth events and programs.",
    keywords: ["actividades juveniles", "gestión de eventos", "software", "programas de verano", "CTeen", "plataforma educativa"],
    publishDate: "2022-11-05T00:00:00Z",
    type: "software",
    client: "CTeen Organization",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "kosher-without-borders",
    img: "Kosher-without-borders",
    alt: "Kosher without borders",
    title: "Kosher without borders",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "dragonfly-by-iNVAP",
    img: "Dragonfly",
    alt: "Dragonly",
    title: "Dragonfly by iNVAP",
    service: "Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "streamlion",
    img: "Streamlion",
    alt: "Streamlion",
    title: "Streamlion",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "adMouse",
    img: "Mouse4",
    alt: "Cladd",
    title: "AdMouse",
    service: "Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "global-gaming-system",
    img: "Global-Gaming-System",
    alt: "Cladd",
    title: "Global Gaming System",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "mousebox",
    img: "Mousebox",
    alt: "Mousebox",
    title: "Mousebox",
    service: "Software and Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "sooning",
    img: "Sooning",
    alt: "Sooning",
    title: "Sooning",
    service: "Software and Hardware Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
  {
    slug: "techni-mobili",
    img: "Techni-Mobili",
    alt: "Techni Mobili",
    title: "Techni Mobili",
    service: "Software Design",
    web: "unitybox.org",
    banner: "project-image.png",
    altBanner: "",
    phrase: "Charity box: Empower your giving with a single tap.",
    years: "2022 - 2023",
    country: "Argentina",
    company: "Unitybox is an innovative platform designed to revolutionize the way people engage in charitable giving. By offering a digital charity box, Unitybox makes it easy for users to give with just a tap, celebrating even the smallest acts of kindness. The platform simplifies the donation process, allowing individuals to make a meaningful impact daily, creating a global community of conscious givers.",
    challenges: [
      {
        icon: "mingcute:cash-line",
        text: "Fewer people carry cash."
      },
      {
        icon: "mingcute:pig-money-line",
        text: "Traditional charity boxes outdated."
      },
      {
        icon: "mingcute:cellphone-line",
        text: "Need for digital donation solutions."
      },
      {
        icon: "mingcute:hand-heart-line",
        text: "Make giving simple and accessible."
      },
    ],
    technologies: [
      {
        icon: "Technologies.svg",
        title: "Figma",
      },
      {
        icon: "Technologies.svg",
        title: "Slack",
      },
      {
        icon: "Technologies.svg",
        title: "Nuxt JS",
      },
      {
        icon: "Technologies.svg",
        title: "Vercel",
      },
      {
        icon: "Technologies.svg",
        title: "Javascript",
      },
      {
        icon: "Technologies.svg",
        title: "Vue JS",
      },
    ],
    resources: [
      {
        icon: "Resource.svg",
        title: "Engeneering team"
      },
      {
        icon: "Resource.svg",
        title: "Front-end development team"
      },
      {
        icon: "Resource.svg",
        title: "Back-end development team"
      },
    ],
    outcome: "The Unitybox project successfully modernized charitable giving by making it digital, simple, and accessible. Users can now donate effortlessly through the platform with just one tap, making small acts of kindness a daily habit. The outcome is a growing global community of givers, each contributing to a collective impact through tiny, consistent actions. Unitybox has redefined what it means to give in today's digital world.",
    outcomeImages: [
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
      {
        img: "project-vertical.png",
        alt: ""
      },
    ]
  },
]